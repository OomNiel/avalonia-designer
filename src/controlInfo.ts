...
const INFO: Record<string, ControlInfo> = {
    ...
    GrumpySheet: {
        ...
        use: 'In the designer, the cell contents are typed in through the Cells row in the Properties panel (Edit cells…), which writes them into the form as <spread:SheetCell> elements and sets Rows/Columns there too, and the same dialog\'s formatting bar sets bold, italics, size, font, colours and alignment for whatever is selected. The sheet is dockable like any panel child — the Dock row pins it to an edge of a DockPanel (Left/Right give it a column of its own, Top/Bottom a band, Fill takes what is left), and the designer wraps it in a DockPanel if it is not already in one. In the app you run the sheet is live: click a cell and type, drag to select a range, click a column letter or a row number to select the whole line, and drag the small square at the bottom-right of the selection to fill a series — 1, 2 becomes 3, 4, 5 …, 2, 4 becomes 6, 8 …, Item1, Item2 becomes Item3, and anything else repeats. Delete clears the selection, F2 or a double click edits the active cell, Esc abandons an edit, and the fx box at the top shows and edits the active cell\'s contents (Ctrl+U or a click puts the caret there). A right-click opens a menu that lines the selection up (left, centre, right, or automatic, for cells or for whole columns and rows), bolds or italicises it, and clears its contents or its formatting. Drag a border between two column letters — or between two row numbers — to give that column or row its own size; the new sizes are readable and writable through ColumnWidths/RowHeights as "3:120,7:60" pairs, and SheetSizeChanged fires once per drag, on release. Scrollbars appear when the sheet is bigger than its space (Show Scrollbars = False turns them off), the wheel scrolls the rows, Shift+wheel the columns, and a wheel over a sheet too short to scroll moves the columns instead. Read what the user typed with GetCell(row, column), write it with SetCell(row, column, text) — both 1-based, as are SelectedFirstRow/SelectedLastRow/SelectedFirstColumn/SelectedLastColumn — and react to every committed change with the CellChanged event; Allow Editing = False turns the sheet into a read-only result grid. A cell whose text starts with "=" is a FORMULA: the text is kept and shown in the fx box and the editor, and the grid draws what it works out to — read it with ValueOf(row, column). The four operators, ^ and &, comparisons, brackets, A1 references, A1:B3 ranges and SUM / AVERAGE (AVG) / MIN / MAX / COUNT / COUNTA / ABS / ROUND / INT / SQRT / MOD / IF / AND / OR / NOT / LEN / UPPER / LOWER / TRIM all work, and IF only evaluates the branch it takes, so =IF(A1=0,0,1/A1) is safe. What cannot be worked out is named: #VALUE!, #NAME?, #REF!, #DIV/0! and #CYCLE! for a cell that reaches itself — never a blank and never a crash.'
    },
};
