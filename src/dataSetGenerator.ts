...
function csPersistMethods(spec: DataSetSpec, t: DataTableSpec): string[] {
    ...
    lines.push('            // Sorting / scrolling recycle the DataGrid row containers, so a container that once');
    lines.push('            // showed the blank add-row can later be bound to an ordinary data row. One shared');
    lines.push('            // pointer handler is attached only while a container shows the add-row (removed again');
    lines.push('            // on UnloadingRow) and it re-checks the row CURRENT DataContext at press time, so a');
    lines.push('            // recycled container never pops the dialog for a reordered data row.');
    lines.push('            System.EventHandler<Avalonia.Input.PointerPressedEventArgs> addRowPointer = (rs, re) =>');
    lines.push('            {');
    lines.push('                if (!re.GetCurrentPoint(grid).Properties.IsLeftButtonPressed) return;');
    lines.push('                var addRow = rs as DataGridRow;');
    lines.push(`                if (addRow == null || !(addRow.DataContext is ${R} d) || !d.IsPlaceholder) return;`);
    ...
    lines.push('                {');
    lines.push('                    e.Row.AddHandler(Avalonia.Input.InputElement.PointerPressedEvent, addRowPointer, Avalonia.Interactivity.RoutingStrategies.Bubble, true);');
    lines.push('                }');
    lines.push('                else');
    lines.push('                {');
    ...
    lines.push('                }');
    lines.push('            };');
    lines.push('            grid.UnloadingRow += (_, e) => e.Row.RemoveHandler(Avalonia.Input.InputElement.PointerPressedEvent, addRowPointer);');
    lines.push('        }');
    lines.push('');
...
function vbPersistMethods(spec: DataSetSpec, t: DataTableSpec): string[] {
    ...
    lines.push('            \' on UnloadingRow) and it re-checks the row CURRENT DataContext at press time, so a');
    lines.push('            \' recycled container never pops the dialog for a reordered data row.');
    lines.push('            Dim addRowPointer As System.EventHandler(Of Avalonia.Input.PointerPressedEventArgs) = Sub(rs, re)');
    lines.push('                If Not re.GetCurrentPoint(grid).Properties.IsLeftButtonPressed Then Return');
    lines.push('                Dim addRow As DataGridRow = TryCast(rs, DataGridRow)');
    lines.push('                If addRow Is Nothing Then Return');
    lines.push(`                Dim addData As ${R} = TryCast(addRow.DataContext, ${R})`);
    lines.push('                If addData Is Nothing OrElse Not addData.IsPlaceholder Then Return');
    lines.push('                re.Handled = True');
    ...
    lines.push('                    row.AddHandler(Avalonia.Input.InputElement.PointerPressedEvent, addRowPointer, Avalonia.Interactivity.RoutingStrategies.Bubble, True)');
    lines.push('                Else');
    ...
    lines.push('                End If');
    lines.push('            End Sub');
    lines.push('            AddHandler grid.UnloadingRow, Sub(s, e) e.Row.RemoveHandler(Avalonia.Input.InputElement.PointerPressedEvent, addRowPointer)');
    lines.push('        End Sub');
    lines.push('');
...
export function generateCs(spec: DataSetSpec, rootNamespace: string): string {
    ...
    const lines: string[] = [];
    lines.push('// Generated by Grumpy\'s WYSIWYG Designer — DataSet designer. Do not edit by hand.');
