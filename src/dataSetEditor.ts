...
function isGeneratedDataSetFile(file: string): boolean {
    // TWO spellings, on purpose: files written before the 2026-09-20 rename carry the old wording, and a
    // generated DataSet that stopped looking generated would lose its editor — and the protection that
    // stops the designer rewriting the user's own code. Both count, for as long as old files are around.
    return /Generated by (?:the Avalonia Designer|Grumpy's WYSIWYG Designer)/.test(readText(file));
}
