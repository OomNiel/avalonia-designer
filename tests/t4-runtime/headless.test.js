...
module.exports = async (t) => {
  ...
  // Defensive: this generated project has been observed to lose its ProjectReference between the
  // write and the build (the driver then fails with "the type or namespace name 'HeadlessApp'
  // could not be found" while the file on disk is three lines short). Re-assert it so the layer
  // either passes or tells us why it cannot.
  const csprojPath = path.join(HARNESS_DIR, 'Harness.csproj');
  if (!/<ProjectReference\b/.test(fs.readFileSync(csprojPath, 'utf8'))) {
