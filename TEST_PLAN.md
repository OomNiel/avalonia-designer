...
### Status 2026-09-10 — full suite green (1908 passed / 0 failed / 0 skipped, 33 s)

- **T2** gained `bundledComponents.test.js`, `chromeProps.test.js`, `multiProps.test.js` plus the
  Data-Image / Browse-picker / palette additions to `codeBehind.test.js` and `dataSet.test.js`.
- **T3** now runs **360** checks (was silently 5 while the fixture was stale) and guards its own
  fixture against `media/designer.js` — keep that assertion when adding a toolbar button or modal.
- **T4** re-asserts the harness `ProjectReference` right before `dotnet run`; the layer passes 14/14.
- **Known gap:** `src/codeBehindCheck.ts` (the “Code Fix…” checker) has **no unit tests** yet. Its
  rules are pure functions over file text, so a T2 suite can drive them with a damaged copy of a
  fixture project (inject damage → `analyzeCodeBehind` → `applyLocalFix` → re-analyse → expect 0).
...
- **Follower bindings** are covered by `t2-logic/follower.test.js` (32 checks): the `.adset` record,
