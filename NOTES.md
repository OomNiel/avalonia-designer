...
### Public GitHub Beta — release hygiene (§67, 2026-09-02)
- **Point 1 (portability)** and **point 2 (git):** see §66 fix 1/2 and below. Repo initialized on `main`, `.gitignore` hardened (`out/`, `host/bin`, `host/obj`, `.poolside/`), initial commit `42f4f09` pushed to **https://github.com/OomNiel/avalonia-designer** (public, via `gh repo create`).
- **Point 3 (metadata):** `package.json` now has `repository` (OomNiel/avalonia-designer), `homepage`, `bugs`, `keywords` (10) and `categories` → `["Other","Visualization"]` — the vsce "repository missing" warning is gone.
