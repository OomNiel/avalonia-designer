...
- **🩺 Code Fix…** — checks the form's code-behind against the `.axaml` and the project's DataSets,
  ...
  `Imports` or bundled helper files. Findings also land in the **PROBLEMS** pane.
- **“Do not edit by hand” notice** — every generated `.axaml` (new project, New Form) starts with
  `<!-- Do NOT edit this file manually - Use the Designer to make changes -->`, and the designer
  re-stamps it on every save. The designer re-formats the form when it writes it, so hand edits are
  lost (see the manual, §4).
- **File / Folder Selector** — drop a path row (box + “…” button) that opens the platform's own
  Open-file / Folder / Save-as dialog and stores the choice in a two-way `SelectedPath`.
  The bundled `PathPicker.cs|.vb` helper is copied into the project on first use, and the row shows a
  page/folder icon (**Show Icon**) so pickers are tellable apart at a glance.
- **💾 Project Backup** — one toolbar click saves the form, the open DataSets and every dirty editor,
  then copies the project into its parent folder as `<Project>_<date>_<time>` (`bin`, `obj`,
  `node_modules` and `.git` are skipped) — a snapshot before a risky change, or to hand over.
- **Menu selectors** — a **Menu Items** row can be a File Selector or Folder Selector, written as a
  real `<chrome:PathPicker>` inside the menu item.
- **Grouped, collapsible Properties** — the sidebar files every control's rows into **Editors**,
  **Layout & size**, **Appearance**, **Text & font**, **Data** and **Behavior** (all colours together,
  all size/dock/anchor/alignment rows together, …), in the same order for every control. Sections
  fold on click and the folded set is remembered per control type.
- **Follow-a-column bindings** — a ComboBox/ListBox/ItemsControl can list one **text column** of a
  table a DataGrid owns, **live** (add/edit/delete in the grid updates the list in place; the
  “+ Add row…” placeholder row is skipped). The table still belongs to the grid, so the editing,
  undo/redo and save-back are untouched.
...
│    - Avalonia 12.1.1 headless (UseHeadless + UseSkia)                              │
...
> **Single Avalonia version.** Both the bundled Previewer Host (**net8.0**, auto-builds on first
> use) and generated projects (**net10.0**) use **Avalonia 12.1.1** — the designer preview renders
> with the same Avalonia your app runs, so what you design is what you get. The designer is
> **opt-in**: `.axaml` files open in the normal text editor by default; use **Avalonia: Open in
> Designer** (or the editor's tab dropdown) to open them in the designer.
...
│   └── logger.ts
├── host/
│   ├── PreviewerHost.csproj # net8.0, Avalonia 12.1.1, headless + Skia
