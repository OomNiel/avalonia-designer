...
'    - Dock-able controls (Menu / Status Bar / grids / images / ...) can instead
'      be DOCKED to the panel's edges: they are placed into the inner DockPanel
...
' NOTE: `Global.` is required for the same reason as AnchorHelper.vb - VB prepends
' RootNamespace to a plain `Namespace AvaloniaChrome`, so the compiled type would be
' <RootNamespace>.AvaloniaChrome.GrumpyPanel and the XAML compiler could not resolve
' it from "using:AvaloniaChrome".
Namespace Global.AvaloniaChrome

    ''' <summary>
    ''' A docking-region panel: a Border frame that hosts a nested DockPanel + free
    ''' body (see the file header). The class exists so forms can carry a real,
    ''' distinct chrome:GrumpyPanel element; its layout is the stock
    ''' DockPanel/Canvas composition inside, so it previews and runs identically.
    ''' </summary>
    Public Class GrumpyPanel
        Inherits Border

        ''' <summary>Marker used by the designer's bundled-component refresh (keeps old copies current).</summary>
        Public Const BundledMarker As String = "GrumpyPanel"
